...
export default async function handler(req, res) {
  if (req.method === "POST") {
    try {
      ...
      await connectDB();
      let { gameWord } = await GameId.findById(gameId);
      ...
      if (gameWord === word) {
        return res
          .status(200)
          .json({ message: "Correct word", letterPosition });
      }
      ...
      return res
        .status(202)
        .json({ message: "Incorrect word", letterPosition });
    } catch (err) {
...
function checkLetterPosition(gameWord, currWord) {
  ...
  for (let i = 0; i < currWord.length; i++) {
    if (
      letterPositionArray[i] === " " &&
      gameWord.indexOf(currWord[i]) !== -1 &&
      gameWordMap.get(currWord[i]) > 0
    ) {
